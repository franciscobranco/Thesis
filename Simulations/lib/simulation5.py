...
def run_simulation():
    ...
    auv_pf_system = sb.DoubleAUVMPFETC(p_target, p0, p1, pf_params=pf_params, cpf_params=cpf_params, etc_type="Time", history=True, dt=dt)
    ...
    all_outputs, _, pf_target, _, pf0, cpf0, _, pf1, cpf1 = auv_pf_system.past_values()
    ...
    for i in range(len(T)):
        if i % frame_rate == 0:
            ...
            ax1[0][0].plot(all_outputs["x_target"][i], all_outputs["y_target"][i], 'go')

            p_r = pg.Path()
            circle_r = pg.Circle(resolution, np.array([all_outputs["x_target"][i], all_outputs["y_target"][i]]), all_outputs["theta_m_target"][i], arc, radius, start)
            ...
            ax1[0][0].legend(['target\'s path', 'target', 'followers\' path', 'follower0', 'follower1'])
            ...
            ax1[1][0].set_xlabel('time [s]')
            ax1[1][0].set_ylabel('Velocity [m/s]')
            ax1[1][0].legend(['follower0', 'follower1'])
            ax1[1][0].grid()

            # Error plot
            ax1[0][1].set_title('Gamma Error plot between 2 Followers')
            difference = []
            ...
            ax1[1][1].scatter(cpf1["broadcasts"], np.full(len(cpf1["broadcasts"]), 1), c='orange', marker='+')
            ax1[1][1].set_xlabel('time [s]')
            ax1[1][1].legend(['follower0', 'follower1'])
            ...
            ax1[0][2].legend(['follower0 y1', 'follower0 s1', 'follower1 y1', 'follower1 s1'])
