...
def plot(paths, num_points, total_time, resolution, T, past_values):
    ...
    for i in range(len(T)):
        if i % frame_rate == 0:
            ...
            ax[0][0].plot(all_outputs["x_target1"][i], all_outputs["y_target1"][i], color='tab:blue', marker='o')
            ...
            ax[0][0].plot(all_outputs["x_target1"][:i], all_outputs["y_target1"][:i], color='tab:blue', linestyle='--', label='_nolegend_')
            ...
            p_r = pg.Path()
            circle_r = pg.Circle(resolution, np.array([all_outputs["x_target1"][i], all_outputs["y_target1"][i]]), all_outputs["theta_m_target1"][i], p_tracker0.path_list[0].arc, p_tracker0.path_list[0].radius, p_tracker0.path_list[0].start)
            p_r.append_path(circle_r)
            p_r.plot_path(ax[0][0])
            ...
            ax[0][2].set_title('Estimation of Trackers\' Position')
            ax[0][2].plot(T[:i], position_error, c='tab:gray')
            #ax[0][2].plot(T[:i], pf_tracker0["s1_geo"][:i])
            #ax[0][2].plot(T[:i], pf_tracker1["y1_geo"][:i])
            #ax[0][2].plot(T[:i], pf_tracker1["s1_geo"][:i])
            ax[0][2].set_xlabel('time [s]')
            ax[0][2].set_ylabel('distance error [m]')
            ax[0][2].grid()
            #ax[0][2].legend(['tracker0 y1', 'tracker0 s1', 'tracker1 y1', 'tracker1 s1'])
            ...
            ax[1][2].plot(T[:i], all_outputs["u_target1"][:i], c='tab:blue')
