...
def plot(paths, num_points, total_time, resolution, T, past_values, Movie=None):
    ...
    if Movie == None:
        ...
        # for i in range(len(T)):
            ...
            ax[0][0].plot(all_outputs["x_pred_target0"][:i], all_outputs["y_pred_target0"][:i], color='tab:gray', linestyle='--', label='_nolegend_')
            ax[0][0].plot(all_outputs["x_pred_target1"][:i], all_outputs["y_pred_target1"][:i], color='tab:gray', linestyle='--', label='_nolegend_')
            ax[0][0].plot(all_outputs["x_pred_target2"][:i], all_outputs["y_pred_target2"][:i], color='tab:gray', linestyle='--', label='_nolegend_')
            ...
            ax[0][0].plot(all_outputs["x_ekf0"][i], all_outputs["y_ekf0"][i], color='purple', marker=(3, 0, 360 * all_outputs["theta_ekf0"][i] / (2*pi) - 90), markersize=11)
            ax[0][0].plot(all_outputs["x_ekf1"][i], all_outputs["y_ekf1"][i], color='purple', marker=(3, 0, 360 * all_outputs["theta_ekf1"][i] / (2*pi) - 90), markersize=11, label='_nolegend_')
            ax[0][0].plot(all_outputs["x_ekf2"][i], all_outputs["y_ekf2"][i], color='purple', marker=(3, 0, 360 * all_outputs["theta_ekf2"][i] / (2*pi) - 90), markersize=11, label='_nolegend_')
            ...
            ax[1][0].plot(T[:i], all_outputs["velocity_circle"][:i], color='tab:red', linestyle='-')
            # ax[1][0].plot(T[:i], mpf_tracker0["velocity"][:i], color='magenta', linestyle='-')
            # ax[1][0].plot(T[:i], mpf_tracker1["velocity"][:i], color='red', linestyle='-')
            ax[1][0].set_xlabel('time [s]')
            ax[1][0].set_ylabel('velocity [m/s]')
            ax[1][0].legend([
                ...
                prop={'size': legend_size})
            ax[1][0].grid()
            ax[1][0].set_xlim([0, T[i]])
            #ax[0][1].set_ylim([-5, 5])

            # Cooperative Formation Control Plot
            ax[2][0].set_title('Cooperative Formation Control')
            ax[2][0].plot(T[:i], cfc_centre[:i], c='tab:red')
            ax[2][0].plot(T[:i], pf_target1["s"][:i], c='tab:orange')
            ax[2][0].scatter(cfc_tracker0["broadcasts"][:i], np.full(len(cfc_tracker0["broadcasts"][:i]), 1.5), c='tab:red', marker='+')
            ax[2][0].scatter(cfc_target1["broadcasts"][:i], np.full(len(cfc_target1["broadcasts"][:i]), 1.5), c='tab:orange', marker='+')
            ax[2][0].set_xlabel('time [s]')
            ax[2][0].set_ylabel('Coordination State $\gamma$')
            ax[2][0].legend([
                'Virtual Circle',
            ...
            for j in range(i):
                ...
                if all_outputs["range21"][j] != None:
                    measurements1[2][0].append(T[j])
                    measurements1[2][1].append(all_outputs["range21"][j])
            ax[0][1].plot(measurements0[0][0][:i], measurements0[0][1][:i], color='tab:blue', linestyle='-')
            ax[0][1].plot(measurements1[0][0][:i], measurements1[0][1][:i], color='tab:blue', linestyle='-', label='_nolegend_')
            ax[0][1].plot(measurements0[1][0][:i], measurements0[1][1][:i], color='tab:orange', linestyle='-')
            ax[0][1].plot(measurements1[1][0][:i], measurements1[1][1][:i], color='tab:orange', linestyle='-', label='_nolegend_')
            ax[0][1].plot(measurements0[2][0][:i], measurements0[2][1][:i], color='tab:green', linestyle='-')
            ax[0][1].plot(measurements1[2][0][:i], measurements1[2][1][:i], color='tab:green', linestyle='-', label='_nolegend_')
            ax[0][1].set_xlabel('time [s]')
            ax[0][1].set_ylabel('distance measure [m]')
            ax[0][1].legend(['Target 0', 'Target 1', 'Target 2'],loc='upper right')
            ax[0][1].set_title('Range-measurements')
            ax[0][1].grid()
            ...
            for j in range(i):
                ...
                error[1].append(np.sqrt(np.power(ekf_target1["x"][j] - all_outputs["x_target1"][j], 2) + np.power(ekf_target1["y"][j] - all_outputs["y_target1"][j], 2)))
                error[2].append(np.sqrt(np.power(ekf_target2["x"][j] - all_outputs["x_target2"][j], 2) + np.power(ekf_target2["y"][j] - all_outputs["y_target2"][j], 2)))
            ax[1][1].plot(T[:i], error[0][:i], linestyle='-', color='tab:blue')
            ax[1][1].plot(T[:i], error[1][:i], linestyle='-', color='tab:orange')
            ax[1][1].plot(T[:i], error[2][:i], linestyle='-', color='tab:green')
            ax[1][1].set_xlabel('time [s]')
            ax[1][1].set_ylabel('distance [m]')
            #ax[1][1].legend(['x_dot', 'y_dot', 'velocity'])
            ax[1][1].set_title('EKF Error')
            ax[1][1].legend(['Target 0', 'Target 1', 'Target 2'], loc='upper right')
            ax[1][1].grid()
            ...
            for j in range(i):
                ...
                position_error[2].append(np.linalg.norm(np.array([all_outputs["x_target2"][j] - all_outputs["x_pred_target2"][j], all_outputs["y_target2"][j] - all_outputs["y_pred_target2"][j]])))
            ax[2][1].set_title('CKF Error')
            ax[2][1].plot(T[:i], position_error[0][:i], linestyle='-', color='tab:blue')
            ax[2][1].plot(T[:i], position_error[1][:i], linestyle='-', color='tab:orange')
            ax[2][1].plot(T[:i], position_error[2][:i], linestyle='-', color='tab:green')
            ax[2][1].set_xlabel('time [s]')
            ax[2][1].set_ylabel('distance [m]')
            ax[2][1].grid()
            ax[2][1].legend(['Target 0', 'Target 1', 'Target 2'], loc='upper right')
